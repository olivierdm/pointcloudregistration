...
void Vision::operator()(const sensor_msgs::ImageConstPtr& msg, const tum_ardrone::filter_stateConstPtr& poseMsg, std::vector<cv::Rect> & rectangles, std::vector<cv::Vec4f> & lines)
{
	...
	getLines(cv_input_ptr, lines);
	detect(cv_input_ptr, rectangles);
...
void Vision::getLines(cv_bridge::CvImagePtr & cv_input_ptr, std::vector<cv::Vec4f> & lines)
{
///
...
void Vision::detect(cv_bridge::CvImagePtr & cv_input_ptr, std::vector<cv::Rect> & rectangles)
{
///
